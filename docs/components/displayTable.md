...
<template>
    <div class="display-info w-99">
        <div class="flex-layout">
            <div class="table-ly info-item" v-for="(item, index) in list" :key="index" :style="{
                ...
                <div class="table-cell px-10 content" :title="infoTableData[item.key]">
                    <div v-if="item.slot">
                        <slot :name="item.slot"></slot>
                    </div>
                    <span v-else>{{
                            infoTableData[item.key] ? infoTableData[item.key] : "-"
                    }}</span>
                </div>
            </div>
        </div>
